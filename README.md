...
★ 會前系統檢查（8 項）
    ├── 麥克風權限 / 語音辨識權限 / 螢幕錄製權限
    ├── 麥克風音訊擷取 / 語音辨識 (zh-TW)
    ├── Claude AI 連接 / Notion API 連接
    └── NotebookLM Bridge → 全部通過 → 開始會議
    ↓
會前準備 UI
    ...
    ↓
會議進行中
    ├── 即時逐字稿（對方 青色 / 我方 黃色 分色顯示）
    ├── ★ Live Partial Results（紫色波形即時辨識文字）
    ├── AI 提詞卡片（🔵本地 / 🟣Claude / 🟠策略）
    ├── TP 追蹤（✅ 偵測到我方已講 / 🔊 偵測中）
    ├── 音訊健康監控（對方/我方串流狀態 badge）
    ├── 系統健康（CPU / 記憶體 / 網路品質）
...
│
├── MeetingCopilot.xcodeproj/           # v4.3.1 build 8
├── MeetingCopilot/
...
# 4. 執行系統檢查 → 會前準備 → 輸入資料或載入 TXT → 選語言 → 開始會議
```
...
| **v4.3.1** | **SystemCheck + Live Partial + 錯誤訊息改善 + 配色統一** | **✅ 目前** |
